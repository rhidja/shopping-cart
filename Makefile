...
PHP_UNIT = $(SYMFONY) php bin/phpunit
...
database-init-test: ## Init database for tests
	...
	$(SYMFONY_CONSOLE) do:da:dr --force --env=test
	$(SYMFONY_CONSOLE) do:da:cr --env=test
	$(SYMFONY_CONSOLE) do:mi:mi --no-interaction --env=test
	$(SYMFONY_CONSOLE) do:fi:lo --no-interaction --env=test
